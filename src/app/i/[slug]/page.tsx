...
export default async function InvitationPage({ params, searchParams }: PageProps) {
  ...
  // 데이터는 삭제하지 않고 화면 노출만 막는다. (스위치: 관리자 페이지에서 청첩장별 토글 content.meta.publicAutoClose)
  // 단, 사이트 관리자(/admin 로그인 쿠키)는 종료 후에도 그대로 열람 가능.
  const publicHidden = (realInvitation as unknown as { public_hidden?: number }).public_hidden === 1;
  const publicAutoClose = invitationContent?.meta?.publicAutoClose === true;
  if (
    isPublicViewClosed({
      enabled: publicAutoClose,
      weddingDate: realInvitation.wedding_date,
      publicHidden,
      isPreview,
