...
/** 이 청첩장 우표를 공개 컬렉션에 노출할지(content.meta.stampPublic). 기본값=false(비공개). admin opt-in용. */
export function stampPublicOf(contentJson: string | null): boolean {
  if (!contentJson) return false
  try {
    const c = JSON.parse(contentJson) as { meta?: { stampPublic?: unknown } }
    return c?.meta?.stampPublic === true || c?.meta?.stampPublic === 1
  } catch {
    return false
  }
}

/** 공개 우표 노출 opt-in 설정/해제(admin). true → 공개 컬렉션에 노출. 기본값(미설정)은 비공개. */
export async function setStampPublic(invitationId: string, isPublic: boolean): Promise<void> {
  const db = await getDB()
  const row = await db.prepare('SELECT content FROM invitations WHERE id = ? LIMIT 1').bind(invitationId).first<{ content: string | null }>()
  let content: Record<string, unknown> = {}
  try {
  ...
  const meta = content.meta && typeof content.meta === 'object' ? (content.meta as Record<string, unknown>) : {}
  if (isPublic) meta.stampPublic = true
...
export async function ensurePostDrawer(invitationId: string): Promise<PostDrawerRow> {
  ...
  // 신규 모임 생성: 계정당 모임 수 상한 체크 (백스톱)
  const owner = await db.prepare('SELECT user_id FROM invitations WHERE id = ?').bind(invitationId).first<{ user_id: string }>()
  if (owner?.user_id) {
    const cnt = await db
      .prepare('SELECT COUNT(*) AS c FROM post_drawers p JOIN invitations i ON p.invitation_id = i.id WHERE i.user_id = ?')
      .bind(owner.user_id)
      .first<{ c: number }>()
    if ((cnt?.c ?? 0) >= POST_DRAWER_MAX_PER_ACCOUNT) {
      throw new Error(`데이드로어 모임은 계정당 최대 ${POST_DRAWER_MAX_PER_ACCOUNT}개까지 만들 수 있어요.`)
    }
  }
  const slug = crypto.randomUUID().replace(/-/g, '')
  await db
...
export async function getStampCollection(): Promise<Stamp[]> {
  const db = await getDB()
  // 공개 컬렉션(우표) = ① 관리자가 직접 추가한 우표(collection_stamps) + ② admin이 공개로 전환한 발행 청첩장 우표.
  //   ②는 기본 비공개(opt-in): meta.stampPublic === true 인 것만 노출. PII(이름)는 노출하지 않음(name:'').
  const admin = (
    await db
      .prepare('SELECT photo, message, wedding_date FROM collection_stamps WHERE IFNULL(hidden,0) = 0 ORDER BY wedding_date DESC, created_at DESC')
      .all<{ photo: string | null; message: string | null; wedding_date: string | null }>()
  ).results || []
  const adminStamps: Stamp[] = admin.map((r) => ({
  ...
  // ② 발행 청첩장 우표: 사진=우표전용사진 or 카카오썸네일, 한마디=stamp_message or 사랑문구, 숨김 제외
  const inv = (
    await db
      .prepare(
        `SELECT i.id AS id, i.content AS content, i.wedding_date AS wedding_date, p.stamp_message AS stamp_message
         FROM invitations i LEFT JOIN post_drawers p ON p.invitation_id = i.id
         WHERE i.is_published = 1`,
      )
      .all<{ id: string; content: string | null; wedding_date: string | null; stamp_message: string | null }>()
  ).results || []
  const invStamps: Stamp[] = inv
    .filter((r) => stampPublicOf(r.content))
    .map((r) => {
      const photo = drawerStampPhotoOf(r.content) || kakaoThumbnailOf(r.content)
      const msg = r.stamp_message && r.stamp_message.trim() ? r.stamp_message.trim() : pickLoveFragment(r.id || r.wedding_date || '')
      return { photo: photo || null, name: '', weddingDate: r.wedding_date ?? null, message: msg }
    })

  // 병합 + 예식일 최신순(날짜 없는 것은 뒤로)
  return [...adminStamps, ...invStamps].sort((a, b) => (b.weddingDate || '').localeCompare(a.weddingDate || ''))
}
