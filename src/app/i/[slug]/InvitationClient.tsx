...
const globalStyles = `
  ...
  .our-kenburns { animation: ourKenBurns 9s cubic-bezier(0.22, 1, 0.36, 1) forwards; }
  .our-kenburns-slow { animation: ourKenBurns 4s cubic-bezier(0.22, 1, 0.36, 1) forwards; }

  /* 타이틀 자간 수축 진입 */
  @keyframes ourTrackIn {
    from { opacity: 0; letter-spacing: 0.55em; filter: blur(3px); }
...
function useScrollAnimation() {
  ...
  useEffect(() => {
    ...
    const isInView = () => {
      ...
      if (rect.bottom <= 40) return false // 아직 위로 안 들어옴/지나감
      // (1) 상단이 뷰포트 82% 지점을 넘어 충분히 들어옴 (일반 섹션 — 너무 이르지 않게)
      const enough = rect.top < viewH * REVEAL_RATIO
      // (2) 하단까지 완전히 뷰포트 안에 들어옴 (짧은 말단 섹션 stranded 방지)
    ...
    // 스크롤 안전망: IntersectionObserver가 중첩 스크롤 컨테이너에서 놓쳐도
    // 실제 스크롤 시 위치를 직접 계산해 확실히 리빌 (짧은 말단 섹션도 stranded 방지)
...
function HighlightedSection({
  ...
  return (
    <div
      ...
      style={{
        ...style,
        opacity: hasAppeared ? (isActive ? 1 : 0.45) : 0,
        transform: hasAppeared ? 'translateY(0)' : 'translateY(20px)',
        filter: hasAppeared ? (isActive ? 'blur(0px) grayscale(0%)' : 'blur(0px) grayscale(18%)') : 'blur(6px) grayscale(0%)',
...
// Staggered Gallery Section - each photo fades in sequentially
function StaggeredGallerySection({ invitation, themeColors, showAllGallery, onShowAllGallery, onOpenLightbox }: {
  invitation: any
  themeColors: ColorConfig
  ...
  return (
    <div ref={ref} className="px-5 py-10" style={{ background: themeColors.cardBg }}>
      {allImages.length > 0 ? (
        <>
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        .our-lightbox--closing {
          animation: our-lb-close 0.35s cubic-bezier(0.4, 0, 0.6, 1) forwards;
        }

        @keyframes our-lb-close {
          from {
            opacity: 1;
            transform: scale(1);
          }
...
function YouTubeLiteSection({ youtube, themeColors, cardBg, onPlay, onStop }: { youtube: any; themeColors: any; cardBg: string; onPlay?: () => void; onStop?: () => void }) {
...
const mockInvitation = {
  ...
  // Gallery display style settings
...
function transformToDisplayData(dbInvitation: Invitation, content: InvitationContent | null): DisplayInvitation {
  ...
  return {
    ...
    accentTextColor: content.accentTextColor,
    bodyTextColor: content.bodyTextColor,
...
function MainPage({ invitation, invitationId, fonts, themeColors, onNavigate, onOpenRsvp, onOpenLightbox, onOpenGuestbookModal, isSample = false, audioRef, navStyle }: PageProps) {
    ...
    <div className="relative h-screen overflow-y-auto">
      ...
      <div className="relative">
        ...
        {invitation.bgm?.enabled && invitation.bgm?.url && audioRef && (
          <MusicToggleInSection audioRef={audioRef} showNotification={invitation.bgm?.showNotification} notificationText={invitation.bgm?.notificationText} />
        )}
      </div>
      ...
      {/* YouTube Section - OUR */}
      <YouTubeLiteSection youtube={(invitation as any).youtube} themeColors={themeColors} cardBg={themeColors.cardBg} onPlay={handleVideoPlay} onStop={handleVideoStop} />
            ...
            <div className="text-center mb-6">
              <p
                ...
                style={{
                  ...
                  backgroundSize: '0% 40%',
                  animation: 'our-highlightDraw 0.8s cubic-bezier(0.22,1,0.36,1) 0.3s both',
                  paddingBottom: '2px',
                }}
              >
        ...
        <div className="mobile-frame">
          <div className="mobile-frame-screen">
            ...
            <div className="mobile-frame-fixed-ui">
              ...
              {/* Music Toggle */}
              <MusicToggle audioRef={audioRef} isVisible={showMusicToggle} showNotification={invitation.bgm?.showNotification} notificationText={invitation.bgm?.notificationText} shouldAutoPlay={
