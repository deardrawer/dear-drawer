...
export async function POST(request: NextRequest) {
  try {
    ...
    // 모임당 사진 수 상한: 기존(실패 아님) + 이번 요청이 상한 초과면 거절
    const existingPhotos = await db
      .prepare("SELECT COUNT(*) AS c FROM guest_upload_files WHERE invitation_id = ? AND status != 'failed'")
      .bind(invitation.id)
      .first<{ c: number }>()
    if ((existingPhotos?.c ?? 0) + files.length > POST_DRAWER_MAX_PHOTOS) {
      return NextResponse.json({ error: `사진은 모임당 최대 ${POST_DRAWER_MAX_PHOTOS}장까지 올릴 수 있어요.` }, { status: 400 })
    }
