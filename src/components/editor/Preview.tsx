...
function MainPage({ invitation, groomName, brideName, fonts, themeColors }: PageProps) {
  ...
  return (
    <div className="relative">
      ...
      {/* Gallery Section */}
      <section className="px-5 py-10" style={{ background: themeColors.cardBg }}>
      ...
      {sectionVisibility.interview && invitation.content.interviews.some(i => i.question || i.answer) && <section className="py-16 px-7 text-center" style={{ background: themeColors.cardBg }}><div className="w-px h-10 mx-auto mb-6" style={{ background: themeColors.divider }} /><p className="text-sm font-light mb-4" style={{ fontFamily: fonts.displayKr, color: themeColors.text, letterSpacing: '1px' }}>결혼에 관한 우리의 이야기</p><div className="w-px h-10 mx-auto mt-6" style={{ background: themeColors.divider }} /></section>}
      {sectionVisibility.interview && invitation.content.interviews.map((interview, index) => interview.question || interview.answer ? <section key={index} className="px-7 py-14" style={{ background: interview.bgClass === 'pink-bg' ? themeColors.sectionBg : themeColors.cardBg }}>{interview.images && interview.images.length > 0 ? interview.images.length === 1 ? (() => { const s = interview.imageSettings?.[0] || { scale: 1, positionX: 0, positionY: 0 }; return <div className="w-full aspect-[4/5] rounded-xl mb-8 overflow-hidden"><div className="w-full h-full bg-cover bg-center transition-transform duration-300" style={{ backgroundImage: `url(${interview.images[0]})`, transform: `scale(${s.scale}) translate(${s.positionX}%, ${s.positionY}%)` }} /></div> })() : <div className="grid grid-cols-2 gap-2 mb-8">{interview.images.slice(0, 2).map((img, i) => { const s = interview.imageSettings?.[i] || { scale: 1, positionX: 0, positionY: 0 }; return <div key={i} className="aspect-square rounded-xl overflow-hidden"><div className="w-full h-full bg-cover bg-center transition-transform duration-300" style={{ backgroundImage: `url(${img})`, transform: `scale(${s.scale}) translate(${s.positionX}%, ${s.positionY}%)` }} /></div> })}</div> : <div className="w-full aspect-[4/5] rounded-xl mb-8 bg-gray-100 flex items-center justify-center"><span className="text-gray-400 text-sm">Interview Image</span></div>}{interview.question && <p className="text-sm mb-5 text-center" style={{ fontFamily: fonts.displayKr, color: themeColors.text, fontWeight: 400 }}>{interview.question}</p>}{interview.answer && <p className="text-[11px] font-light leading-[2.2]" style={{ fontFamily: fonts.displayKr, color: themeColors.text }} dangerouslySetInnerHTML={{ __html: interview.answer.replace(/\n/g, '<br/>') }} />}</section> : null)}

      {/* Guidance & Info Section - 통합 섹션 */}
      {sectionVisibility.guidance && (
        <section className="px-6 py-14" style={{ background: themeColors.background }}>
      ...
      {/* Thank You Section */}
      <section className="min-h-[300px] flex flex-col justify-center items-center text-center px-7 py-20" style={{ background: themeColors.sectionBg }}>
