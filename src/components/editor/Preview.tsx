  ...
  return (
    <div>
      ...
      <section id="preview-invitation" className="px-7 py-10 text-center" style={{ background: themeColors.background }}>
        ...
        <div className={`py-5 mb-9 ${isEmpty(invitation.content.quote?.text) ? 'opacity-50' : ''}`} style={{ borderTop: `1px solid ${themeColors.divider}`, borderBottom: `1px solid ${themeColors.divider}` }}><p className="text-[13px] font-light leading-[1.9] mb-2" style={{ fontFamily: fonts.displayKr, color: themeColors.highlight || themeColors.primary }} dangerouslySetInnerHTML={{ __html: parseHighlight(invitation.content.quote?.text || SAMPLE_QUOTE.text) }} />{!invitation.content.quote?.hideAuthor && <p className="text-[11px] font-light" style={{ color: themeColors.gray }}>{invitation.content.quote?.author || SAMPLE_QUOTE.author}</p>}</div>
...
const FamilyMainPage = memo(function FamilyMainPage({ invitation, groomName, brideName, fonts, themeColors }: PageProps) {
  ...
  return (
    <div className="relative">
      {/* Full Height Divider 0 - 부모님소개 상단 */}
      <section
      ...
      {/* Why We Chose Each Other - Title Divider (Divider 1) */}
