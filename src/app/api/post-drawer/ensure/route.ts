...
import { ensurePostDrawer, stampHiddenOf } from '@/lib/postDrawer'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 관리자가 우표를 숨긴 청첩장은 서랍을 열 수 없다(소유자 본인 포함)
    if (stampHiddenOf(owned.invitation.content)) {
      return NextResponse.json({ error: '현재 이용할 수 없는 서랍입니다.' }, { status: 403 })
    }
