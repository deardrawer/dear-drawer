        ...
        <div className="rsvp-ticket-flash" style={{ background: tc.cardBg, border: `1px solid ${cdiv}`, position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="rsvp-form-animated" style={{ padding: '16px 20px 20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap" style={{ wordBreak: 'keep-all' }}>
                  ...
                  {((side === 'groom' && (invitation.rsvpSideDetailOptions?.groomMother ?? true)) || (side === 'bride' && (invitation.rsvpSideDetailOptions?.brideMother ?? true))) && (
                    <button onClick={() => setSideDetail('mother')} style={{ flex: 1, fontFamily: fonts.body, fontSize: '10px', padding: '10px', border: `1px solid ${sideDetail === 'mother' ? tc.accent : cdiv}`, background: sideDetail === 'mother' ? tc.accent : 'transparent', color: sideDetail === 'mother' ? '#FFFFFF' : (tc.cardText || tc.text), cursor: 'pointer', transition: 'all 0.3s' }}>
            ...
            {invitation.rsvpMealOption && attendance === 'yes' && (
              <div>
                <span style={{ fontFamily: fonts.body, fontSize: '12px', color: tc.cardGray || tc.gray, display: 'block', marginBottom: '6px' }}>식사 여부</span>
                <div className="grid grid-cols-2 gap-2">
                  {([{ v: 'yes' as const, l: '식사 예정' }, { v: 'no' as const, l: '식사 안 함' }]).map(opt => (
                    <button key={opt.v} onClick={() => setMealAttendance(mealAttendance === opt.v ? '' : opt.v)}
                      style={{ fontFamily: fonts.displayKr, fontSize: '10px', letterSpacing: '2px', padding: '11px', border: `1px solid ${mealAttendance === opt.v ? tc.accent : cdiv}`, background: mealAttendance === opt.v ? tc.accent : 'transparent', color: mealAttendance === opt.v ? '#FFFFFF' : (tc.cardText || tc.text), cursor: 'pointer', transition: 'all 0.3s' }}>
                      {opt.l}
                    </button>
                  ))}
                </div>
            ...
            {invitation.rsvpShuttleOption && attendance === 'yes' && (
              <div>
                <span style={{ fontFamily: fonts.body, fontSize: '12px', color: tc.cardGray || tc.gray, display: 'block', marginBottom: '6px' }}>대절버스 이용 여부</span>
                <div className="grid grid-cols-2 gap-2">
                  {([{ v: 'yes' as const, l: '이용 예정' }, { v: 'no' as const, l: '이용 안 함' }]).map(opt => (
                    <button key={opt.v} onClick={() => setShuttleBus(shuttleBus === opt.v ? '' : opt.v)}
                      style={{ fontFamily: fonts.displayKr, fontSize: '10px', letterSpacing: '2px', padding: '11px', border: `1px solid ${shuttleBus === opt.v ? tc.accent : cdiv}`, background: shuttleBus === opt.v ? tc.accent : 'transparent', color: shuttleBus === opt.v ? '#FFFFFF' : (tc.cardText || tc.text), cursor: 'pointer', transition: 'all 0.3s' }}>
                      {opt.l}
                    </button>
                  ))}
                </div>
            ...
            {invitation.rsvpAfterPartyOption && attendance === 'yes' && (
              <div>
                <span style={{ fontFamily: fonts.body, fontSize: '12px', color: tc.cardGray || tc.gray, display: 'block', marginBottom: '6px' }}>애프터파티 참석 여부</span>
                <div className="grid grid-cols-2 gap-2">
                  {([{ v: 'yes' as const, l: '참석' }, { v: 'no' as const, l: '불참' }]).map(opt => (
                    <button key={opt.v} onClick={() => setAfterParty(afterParty === opt.v ? '' : opt.v)}
                      style={{ fontFamily: fonts.displayKr, fontSize: '10px', letterSpacing: '2px', padding: '11px', border: `1px solid ${afterParty === opt.v ? tc.accent : cdiv}`, background: afterParty === opt.v ? tc.accent : 'transparent', color: afterParty === opt.v ? '#FFFFFF' : (tc.cardText || tc.text), cursor: 'pointer', transition: 'all 0.3s' }}>
                      {opt.l}
                    </button>
                  ))}
                </div>
            ...
            <textarea value={rsvpMessage} onChange={e => setRsvpMessage(e.target.value)} placeholder={invitation.rsvpMessagePlaceholder || "전하고 싶은 말 (선택)"} rows={2}
              ...
              style={{ ...inputStyle, resize: 'none' as const }} />
            <button onClick={handleSubmit} disabled={submitting || !name.trim() || !attendance || (invitation.rsvpPhoneOption && phone.length > 0 && phone.length < 4) || (invitation.rsvpSideDetail && !!side && !sideDetail)} className="rsvp-confirm-btn"
              style={{ width: '100%', fontFamily: fonts.displayKr, fontSize: '10px', letterSpacing: '3px', padding: '13px', background: tc.accent, color: '#FFFFFF', border: 'none', cursor: 'pointer', opacity: (submitting || !name.trim() || !attendance || (invitation.rsvpSideDetail && !!side && !sideDetail)) ? 0.4 : 1 }}>
              Confirm
            </button>
          </div>
        </div>
