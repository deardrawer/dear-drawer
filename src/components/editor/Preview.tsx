        ...
        <section id="preview-couple-profile" className="px-6 py-16" style={{ background: '#ffffff' }}>
          <div className="mb-8" style={{ textAlign: parentIntro?.groom?.textAlign ?? invitation.parentIntroTextStyle?.textAlign ?? 'left' }}>
            ...
            <h3 className="text-lg" style={{ fontFamily: fonts.displayKr, color: themeColors.text }}>
              {parentIntro?.groom?.childOrder === '표기안함' ? '' : (parentIntro?.groom?.childOrder || '첫째')}아들 <span style={{ color: themeColors.highlight || themeColors.primary }}>{groomName?.slice(1) || groomName}</span> 결혼합니다
            </h3>
          </div>
          {/* 이미지 */}
      ...
      {parentIntro?.bride?.enabled !== false && (
        <section className="px-6 py-16" style={{ background: '#ffffff' }}>
          <div className="mb-8" style={{ textAlign: parentIntro?.bride?.textAlign ?? invitation.parentIntroTextStyle?.textAlign ?? 'right' }}>
            ...
            <h3 className="text-lg" style={{ fontFamily: fonts.displayKr, color: themeColors.text }}>
              {parentIntro?.bride?.childOrder === '표기안함' ? '' : (parentIntro?.bride?.childOrder || '첫째')}딸 <span style={{ color: themeColors.highlight || themeColors.primary }}>{brideName?.slice(1) || brideName}</span> 결혼합니다
            </h3>
          </div>
          {/* 이미지 */}
          <div className="w-full aspect-[4/3] bg-gray-100 mb-8 overflow-hidden">
