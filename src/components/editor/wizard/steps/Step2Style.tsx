...
export default function Step2Style({ templateId, invitationId }: Step2StyleProps) {
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* 글자 크기 (제목 → 본문 → 이름·날짜·정보 순) */}
      {!isFilm && !isRecord && !isMagazine && (() => {
        const tId = templateId || invitation.templateId
        const isOur = tId === 'narrative-our' || tId === 'narrative-family'  // 제목·정보 카테고리 슬라이더 지원(OUR·FAMILY)
        const ScaleSlider = ({ field, label, desc }: { field: 'titleFontScale' | 'bodyFontScale' | 'infoFontScale'; label: string; desc: string }) => (
          <div className="rounded-xl border border-gray-200 p-4">
            <p className="text-sm font-medium text-gray-900 mb-1">{label}</p>
            <p className="text-xs text-gray-400 mb-3">{desc}</p>
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-400 shrink-0">작게</span>
              <input
                type="range"
                min={0.8}
                max={1.5}
                step={0.02}
                value={invitation[field] ?? 1}
                onChange={(e) => updateField(field, parseFloat(e.target.value))}
                className="flex-1 accent-gray-900"
              />
              <span className="text-xs text-gray-400 shrink-0">크게</span>
              <span className="text-sm font-semibold text-gray-900 w-12 text-right tabular-nums shrink-0">{Math.round((invitation[field] ?? 1) * 100)}%</span>
            </div>
            {Math.abs((invitation[field] ?? 1) - 1) > 0.001 && (
              <button type="button" onClick={() => updateField(field, 1)} className="mt-2 text-xs text-gray-400 hover:text-gray-700 underline">
                기본 크기로
              </button>
            )}
          </div>
        ...
        return (
          <section className="space-y-4">
            ...
            <p className="text-sm text-blue-600"><svg className="w-3.5 h-3.5 text-gray-900 inline -mt-0.5 mr-0.5" viewBox="0 0 24 24" fill="rgba(0,0,0,0.1)" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4" /><path d="M12 8h.01" /></svg> 항목별로 글자 크기를 조절해요.</p>

            {isOur && ScaleSlider({ field: 'titleFontScale', label: '제목 · 헤더 크기', desc: '섹션 제목 · 인터뷰 질문 · 스토리 제목 · 라벨 크기를 조절해요.' })}
            {ScaleSlider({ field: 'bodyFontScale', label: '본문 크기', desc: '인사말 · 소개 · 스토리 · 인터뷰 · 감사인사 · 안내 본문 크기를 조절해요.' })}
            {isOur && ScaleSlider({ field: 'infoFontScale', label: '이름 · 날짜 · 정보 크기', desc: '예식 날짜 · 시간 · 장소 · 부모님/프로필 이름 크기를 조절해요.' })}
          </section>
        )
      })()}
        ...
        {(bgm?.enabled || bgm?.url) && (
          <div className="space-y-4">
            ...
            {bgm?.autoplay && ['narrative-our', 'narrative-family'].includes(templateId || invitation.templateId || '') && (
              <div className="p-3 bg-gray-50 rounded-lg space-y-2">
                <p className="text-sm text-gray-700 font-medium">재생 시작 페이지</p>
                <div className="flex gap-2">
                  <button
                    type="button"
                    className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-all ${
                      (bgm?.startPage || 'main') === 'intro'
