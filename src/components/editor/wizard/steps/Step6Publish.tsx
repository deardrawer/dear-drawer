...
export default function Step6Publish({
  ...
  const { invitation, validateWizardStep, updateNestedField } = useEditorStore()
  ...
  useEffect(() => {
    if (!slug && invitation) {
      const groomName = invitation.groom.name?.replace(/\s/g, '') || 'groom'
      const brideName = invitation.bride.name?.replace(/\s/g, '') || 'bride'
      const dateStr = invitation.wedding.date?.replace(/-/g, '').slice(4) || ''
  ...
  const handleSlugChange = (value: string) => {
    ...
    setSlug(normalized)
    setCheckResult(null)
    setIsApplied(false) // 수정 시 적용 상태 해제
  }
    ...
    <div className="p-6 space-y-8">
      ...
      <section className="space-y-4">
        ...
        <p className="text-sm text-blue-600">
          ...
          신랑신부 이름과 날짜 조합을 추천드려요. 예) minjun-yuna-0321
        </p>
