...
export async function getStampCollection(): Promise<Stamp[]> {
  const db = await getDB()
  // 공개 컬렉션(우표) = 관리자가 신랑신부 동의 하에 직접 승인·추가한 우표(collection_stamps)만 노출.
  // 모든 발행 청첩장을 자동 수집하지 않는다(과거 동작 제거). 하나씩 승인해 올린 것만 배포.
  // PII(groom/bride_name)·전체 content는 애초에 SELECT하지 않음(우표 최소필드만).
  const admin = (
    await db
