...
const globalStyles = `
  ...
  .line-expand-divider {
    ...
    justify-content: center;
    padding: 50px 24px;
    background: linear-gradient(to bottom, #ffffff 0%, #faf8f6 50%, #ffffff 100%);
  }
  ...
  .line-expand-right {
    ...
    top: 50%;
    height: 1px;
    background: linear-gradient(to var(--direction), rgba(212, 165, 165, 0.6), transparent);
    transform: translateY(-50%);
  }
...
function ParentIntroSection({
  ...
  return (
    ...
    >
      {/* Title */}
      <div className={`${side === 'groom' ? 'text-left' : 'text-right'} mb-8`}>
        <p
          ...
          {quote.split(' ').map((word, index) => (
            <span
              ...
              style={{
                ...
                transition: `opacity 0.8s ease-out ${0.3 + index * 0.4}s, transform 0.8s ease-out ${0.3 + index * 0.4}s`,
                marginRight: '0.3em',
              }}
            >
              {word}
    ...
    >
      ...
      {buttonText && buttonUrl && (
        <button
          ...
          className="mt-4 px-5 py-2.5 rounded-full typo-caption transition-all hover:opacity-80"
          style={{
...
function MainPage({ invitation, invitationId, fonts, themeColors, onNavigate, onOpenRsvp, onOpenLightbox, audioRef, showMusicToggle, shouldAutoPlay }: PageProps) {
  ...
  return (
    <div className="relative">
      ...
      {/* Our Story Title Section - FAMILY 템플릿: 신부 부모님 소개 아래 */}
      ...
      {/* Interview Title Section - FAMILY 템플릿: 서로를 선택한 이유 아래 */}
      ...
      {invitation.sectionVisibility?.guestbook !== false && (
        <AnimatedSection className="px-5 py-14 pb-20 text-center" style={{ background: themeColors.cardBg }}>
          ...
          <div className="max-w-[300px] mx-auto mb-9">
            <p className="typo-body mb-4 min-h-[40px]" style={{ fontFamily: fonts.displayKr, color: themeColors.text }}>{invitation.content.guestbookQuestions[currentQuestionIndex] || '두 사람에게 하고 싶은 말을 남겨주세요'}</p>
...
function InvitationClientContent({ invitation: dbInvitation, content, isPaid, isPreview = false, overrideColorTheme, overrideFontStyle, skipIntro = false }: InvitationClientProps) {
  ...
  // Show music toggle always for FAMILY template
  const showMusicToggle = true
                ...
                >
                  ...
                  {currentPage === 'intro' ? (
                    ...
                    <MainPage
                      ...
                      shouldAutoPlay={currentPage === 'main'}
                    />
                  )}
