...
export interface InvitationContent {
  ...
  colorTheme: 'classic-rose' | 'modern-black' | 'romantic-blush' | 'nature-green' | 'luxury-navy' | 'sunset-coral' | 'custom' | 'film-dark' | 'film-light' | 'record-coral' | 'record-rose' | 'record-peach' | 'record-bw' | 'record-lilac' | 'record-mint'
  ...
  sharedRsvpEnabled?: boolean    // 별도 RSVP 링크(/i/{slug}/rsvp) 활성화 (기본 false)
...
const createDefaultInvitation = (template: Template): InvitationContent => ({
  ...
  content: {
    ...
    quote: template.id === 'narrative-our'
      ...
      : template.id === 'narrative-family'
      ? { text: 'We warmly invite you to our wedding.', author: '' }
      : template.id === 'narrative-film'
    ...
    interviews: template.id === 'narrative-film'
      ...
      : (template.id === 'narrative-our' || template.id === 'narrative-family')
      ? [
          { question: '상대방의 첫인상은 어땠나요?', answer: '처음 본 순간, 이 사람이다 싶었어요.\n말로 설명하기 어려운 느낌이었는데,\n대화를 나눌수록 확신이 들었습니다.\n서로의 눈을 바라보며 웃던\n그 순간을 잊을 수 없어요.', images: [], imageSettings: [], bgClass: 'pink-bg' },
          { question: '결혼을 결심하게 된 계기는?', answer: '함께 있을 때 가장 나다울 수 있었어요.\n아무리 힘든 일이 있어도\n이 사람 곁에 있으면 괜찮아지더라구요.\n평생 이 사람과 함께라면\n어떤 일이든 해낼 수 있을 것 같았습니다.', images: [], imageSettings: [], bgClass: 'white-bg' },
          { question: '앞으로의 결혼생활 계획은?', answer: '서로를 존중하고 배려하며 살고 싶어요.\n작은 일상에서도 감사함을 잊지 않고,\n함께 웃으며 나이 들어가고 싶습니다.\n무엇보다 서로의 꿈을 응원하는\n부부가 되고 싶어요.', images: [], imageSettings: [], bgClass: 'pink-bg' },
        ]
      : [
